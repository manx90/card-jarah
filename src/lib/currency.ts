...
export const KWD_PRICE_REGEX = /^\d+(\.\d{1,3})?$/;

export function parseKwdPrice(raw: string): string {
  const trimmed = raw.trim();
  return KWD_PRICE_REGEX.test(trimmed) ? trimmed : "0";
}

/** عرض السعر للمستخدم: 12.500 د.ك */
export function formatPriceKwd(amount: string | number): string {
...
export function formatAmountForCbk(priceDecimalString: string): string {
